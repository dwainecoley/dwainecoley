# Hey, I'm Dwaine 👋

### Cybersecurity | GRC | Cloud Security | Technical

A lover of all things tech. Avid Chess player! Junior cybersecurity professional building hands-on experience across GRC (Governance Risk & Compliance), blue team operations, and cloud fundamentals. For fun, I am pursuing the Google Cybersecurity Certificate, CompTIA Security+, and GRC Mastery.

---

## 🔐 Security Audit Lab

| Project | Description | Status |
|---------|-------------|--------|
| [Security Audit – Botium Toys](https://github.com/dwainecoley/cybersecurity-portfolio/tree/main/projects/botium-toys-audit) | Conducted an internal security audit using NIST CSF. Assessed controls, identified gaps, and produced a compliance checklist. | ✅ Complete |
---

## 📚 Training & Certifications

| Course | Provider | Status |
|--------|----------|--------|
| Google Cybersecurity Certificate | Google / Coursera | 🔄 In Progress |
| CompTIA Security+ | Udemy | 🔄 In Progress |
| GRC Mastery | grcmastery.com | 🔄 In Progress |
| TryHackMe Pre-Security | TryHackMe | 🔄 In Progress |

---

## 🛠️ Tools & Frameworks

`Wireshark` `Kali Linux` `VirtualBox` `NIST CSF` `ISO 27001` `HTML` `TCP/IP`

---

## 🤳 Connect with me

[LinkedIn](https://www.linkedin.com/in/dwainecoley1/) | [Email](mailto:dwainecoley.us@gmail.com)
